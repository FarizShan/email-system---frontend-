...
const Trash = () => {
  ...
  const readTrashEmail = (emailList, index) => {
    if (index >= emailList.length) {
      speak("No more emails to read in trash. Returning to main menu.", () => {
        phaseRef.current = "main";
  ...
  useEffect(() => {
    const initialize = () => {
      const welcomeMessage = "Welcome to InVision Trash. Available commands: 'go to trash', 'menu', 'logout', or 'help' for command list.";
              ...
              trashEmails.map((email) => (
                ...
                >
                  <div className="trash-email-header">
                    <div className="trash-email-sender-wrapper">
                      <span className="trash-email-initial">{getInitial(email.from)}</span>
                      <span className="trash-email-sender">{email.from}</span>
                    </div>
